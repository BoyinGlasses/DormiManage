...
public sealed class ShellViewAnimationTests
{
    ...
    [Fact]
    public void Main_content_offsets_expand_and_shrink_with_sidebar()
    {
        var settingsType = GetSettingsType();

        var sidebarWidth = GetDouble(settingsType, "SidebarWidth");
        var closedOffset = GetDouble(settingsType, "ContentClosedOffset");
        var openOffset = GetDouble(settingsType, "ContentOpenOffset");

        Assert.Equal(0d, closedOffset);
        Assert.Equal(sidebarWidth, openOffset);
    }
